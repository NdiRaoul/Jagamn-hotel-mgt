...
-- Drops and recreates all tables. RLS enabled on all tables.
-- ============================================================
...
-- guest_profiles: own row only
create policy "guest_profiles_select_own"
  on guest_profiles for select using (auth.uid() = id);

create policy "guest_profiles_insert_own"
  on guest_profiles for insert with check (auth.uid() = id);

create policy "guest_profiles_update_own"
  on guest_profiles for update using (auth.uid() = id);
