  ...
  if (!booking) {
    return (
      <div className="text-center py-20">
        <h2 className="manrope-bold text-2xl text-jagamn-primary mb-4">Booking not found</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="relative h-[400px] rounded-md overflow-hidden shadow-sm group">
            <Image src={roomImage} alt={roomName} fill className="object-cover" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-md border-l-4 border-l-jagamn-primary p-8 shadow-sm border-r border-t border-b border-gray-100">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">Check-in</p>
            ...
            <div className="bg-white rounded-md border-l-4 border-l-jagamn-secondary/25 p-8 shadow-sm border border-gray-100">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">Check-out</p>
              <h3 className="manrope-bold text-3xl text-jagamn-primary mb-1">
          ...
          <div className="bg-white rounded-md p-8 shadow-sm border border-gray-100 flex items-center justify-between">
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Reservation Reference</p>
              <h3 className="manrope-bold text-xl text-jagamn-primary font-mono">{booking.booking_ref}</h3>
            </div>
            <button
              onClick={copyRef}
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-md p-10 shadow-sm border border-gray-100 space-y-10">
            <h3 className="manrope-bold text-xl text-jagamn-primary">Stay Summary</h3>
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-500">{nights} Night{nights !== 1 ? "s" : ""} × ${booking.room_price_per_night}</span>
                <span className="font-bold text-jagamn-primary">${roomTotal.toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-500">Palace Resort Fee</span>
                <span className="font-bold text-jagamn-primary">${resortFee}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-500">Taxes (12%)</span>
                <span className="font-bold text-jagamn-primary">${taxAmount}</span>
              </div>
            </div>
            <div className="pt-6 border-t border-gray-50 flex justify-between items-end">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Total</span>
              <span className="manrope-bold text-3xl text-jagamn-primary">${booking.total_amount.toLocaleString()}</span>
            </div>
            <div className="bg-jagamn-neutral rounded-md p-4 flex items-center gap-3">
              ...
              <span className="text-[10px] font-bold text-jagamn-secondary uppercase tracking-widest">
                {booking.payment_status === "paid" ? "Payment Completed" : booking.payment_status}
              </span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-md p-10 shadow-sm border border-gray-100 border-l-4 border-l-red-100">
            <div className="flex items-center gap-2 mb-8">
              ...
              <h3 className="manrope-bold text-xl text-jagamn-primary">Cancellation Policy</h3>
            </div>
            <div className="space-y-6">
              {[
                { label: "> 48 Hours Prior", value: "Full Refund", color: "bg-jagamn-primary" },
                { label: "24 - 48 Hours Prior", value: "50% Refund", color: "bg-jagamn-tertiary" },
                { label: "< 24 Hours Prior", value: "No Refund", color: "bg-red-200" },
              ].map((policy, idx) => (
                <div key={idx} className="flex items-center justify-between text-sm">
                  ...
                  <span className={cn("font-bold", idx === 2 ? "text-red-500" : "text-[#412000]")}>
                    {policy.value}
                  </span>
                </div>
              ))}
      ...
      <Dialog open={isCancelModalOpen} onOpenChange={setIsCancelModalOpen}>
        <DialogContent className="max-w-lg p-0 overflow-hidden border-0 rounded-xl shadow-2xl">
          <div className="bg-jagamn-primary p-6 text-white">
            <DialogHeader>
              <DialogTitle className="manrope-bold text-2xl text-white">Cancel Reservation</DialogTitle>
          ...
          <div className="p-8 space-y-8 bg-white">
            <div className="bg-[#FFF5F5] border-l-4 border-red-500 p-5 flex gap-4">
              ...
              <div className="space-y-1">
                <h4 className="manrope-bold text-xs text-red-600">Cancellation Policy</h4>
                <p className="text-[11px] text-gray-500 leading-relaxed">
                  {rt?.cancellation_policy || "Cancellations may be subject to fees depending on timing."}
                </p>
              </div>
            </div>
            <div className="flex gap-4 pt-2">
              <Button variant="outline" onClick={() => setIsCancelModalOpen(false)} className="flex-1 h-12 border-gray-100 text-jagamn-primary font-bold hover:bg-gray-50">
                Keep Booking
              </Button>
