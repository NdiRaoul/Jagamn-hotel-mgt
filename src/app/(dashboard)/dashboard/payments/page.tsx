...
// ─── Inner card form (needs Stripe hooks — must be inside <Elements>) ────────
function AddCardForm({
  ...
  async function handleSave() {
    ...
    if (confirmErr) {
      onError(confirmErr.message ?? "Card setup failed");
      setSaving(false);
      return;
    }

    // 3. Save the verified PaymentMethod to our DB
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-[10px] text-gray-400">
        Your card is securely tokenised by Stripe. We never store raw card
        numbers.
      </p>
      <Button
        onClick={handleSave}
        disabled={saving || !stripe}
        className="w-full h-12 bg-jagamn-primary text-white font-bold"
      >
...
export default function PaymentsPage() {
  ...
  // Non-card methods (mobile, google, apple) still go through this path
  async function handleAddNonCardMethod() {
    setSaveError(null);
    setSaving(true);
    const {
      data: { user },
    ...
    <div className="space-y-12 max-w-6xl pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-jagamn-primary rounded-md p-12 text-white flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl shadow-jagamn-primary/30">
        ...
        <div className="flex items-center gap-4">
          <Button
            variant="outline"
            className="h-14 px-10 bg-white/5 border-white/10 text-white hover:bg-white/10 gap-2"
          >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="My MTN"
                  className="w-full bg-gray-50 border border-gray-100 rounded-md px-3 h-11 text-sm focus:outline-none focus:border-jagamn-primary"
                />
              </div>
              <Button
                onClick={handleAddNonCardMethod}
                disabled={saving}
                className="w-full h-12 bg-jagamn-primary text-white font-bold"
              >
                {saving ? "Saving..." : "Save Mobile Money"}
              </Button>
            </div>
          )}
          ...
          {(addType === "google" || addType === "apple") && (
            <div className="space-y-4">
              ...
              >
                {saving
                  ? "Saving..."
                  : `Add ${addType === "google" ? "Google Pay" : "Apple Pay"}`}
              </Button>
            </div>
          )}
        </DialogContent>
