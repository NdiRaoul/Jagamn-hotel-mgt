...
export default function PayrollPage() {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Jagamn_Payroll_${payPeriod.toUpperCase()}.csv`,
    );
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div className="space-y-2 text-center lg:text-left">
            ...
            <p className="text-slate-400 font-medium max-w-2xl text-sm md:text-base">
              Review and finalize the employee compensation schedules for the
              current billing cycle.
            </p>
          </div>

          <div className="flex flex-col gap-2 w-full lg:w-auto">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-center lg:text-right lg:mr-1">
              Pay Period
            </p>
            <Select value={payPeriod} onValueChange={setPayPeriod}>
              ...
              <SelectContent className="rounded-xl border-gray-100 shadow-2xl">
                <SelectItem value="oct-2024" className="manrope-bold">
                  October 2024
                </SelectItem>
                <SelectItem value="sep-2024" className="manrope-bold">
                  September 2024
                </SelectItem>
                <SelectItem value="aug-2024" className="manrope-bold">
                  August 2024
                </SelectItem>
              </SelectContent>
            </Select>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 md:p-8 border-b border-gray-50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon"
                    className={cn(
                      "w-9 h-9 md:w-10 md:h-10 rounded-xl transition-all",
                      roleFilter !== "all"
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                className="w-9 h-9 md:w-10 md:h-10 rounded-xl text-slate-400 hover:text-[#0D2137] hover:bg-gray-50"
              >
            ...
            <table className="w-full min-w-[800px] text-left">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-50">
                  ...
                  <th className="px-6 md:px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">
                    Role
                  </th>
                  <th className="px-6 md:px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">
                    Gross Salary
                  </th>
                  <th className="px-6 md:px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">
                    Deductions
                ...
                {payrollData.map((staff) => (
                  ...
                  >
                    ...
                    <td className="px-6 md:px-8 py-5 md:py-6 text-right">
                      <span className="manrope-bold text-sm md:text-base text-[#0D2137]">
                        $
                        {staff.gross.toLocaleString(undefined, {
                    ...
                    <td className="px-6 md:px-8 py-5 md:py-6 text-right">
                      <span className="manrope-bold text-sm md:text-base text-red-500">
                        ($
                        {staff.deductions.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                        })}
                        )
                      </span>
                    ...
                    <td className="px-6 md:px-8 py-5 md:py-6 text-right">
                      <span className="manrope-bold text-sm md:text-base text-[#0D2137]">
                        $
                        {staff.net.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                        })}
                      </span>
                    </td>
          ...
          <div className="p-6 md:p-8 bg-gray-50/30 border-t border-gray-50 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-xs font-bold text-slate-400">
              Showing <span className="text-[#0D2137]">1-4</span> of{" "}
              <span className="text-[#0D2137]">142</span> staff members
            </p>
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <Button
                variant="outline"
                className="flex-1 sm:flex-none h-10 px-5 rounded-xl border-gray-200 text-slate-400 manrope-bold text-xs hover:bg-white hover:text-[#0D2137] shadow-sm"
              >
                Previous
              </Button>
              <Button
