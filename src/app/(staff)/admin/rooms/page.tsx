...
// --- Mock Data ---
const ROOMS_DATA = [
  { id: "101", type: "Deluxe Suite", guest: "Mr. Julian Thorne", status: "occupied", lastUpdated: "10 mins ago" },
  { id: "102", type: "Executive King", guest: "Ready for Check-in", status: "available", lastUpdated: "2 hours ago" },
  { id: "103", type: "Standard Twin", guest: "Cleaning in Progress", status: "cleaning", lastUpdated: "Just now" },
  { id: "104", type: "Garden View", guest: "Arriving 18:00", status: "reserved", lastUpdated: "1 hour ago" },
  { id: "105", type: "Royal Suite", guest: "AC Repair Required", status: "maintenance", lastUpdated: "4 hours ago" },
  { id: "201", type: "Sky Loft", guest: "Lady Genevieve", status: "occupied", lastUpdated: "30 mins ago" },
  { id: "202", type: "Executive King", guest: "Ready for Check-in", status: "available", lastUpdated: "5 hours ago" },
  { id: "203", type: "Deluxe Twin", guest: "Departure at 11:00", status: "cleaning", lastUpdated: "15 mins ago" },
];
...
export default function RoomsPage() {
  ...
  const stats = useMemo(() => {
    ...
    return { occupancy: Math.round((occupied / total) * 100), needsCleaning: cleaning };
  }, [rooms]);
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-10">
        <div className="max-w-xl space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            <Button onClick={() => alert("Bulk Update mode enabled.")} className="h-12 px-6 bg-[#0D2137] text-white manrope-bold rounded-xl flex items-center gap-2 shadow-lg hover:scale-[1.02] transition-all">
          ...
          return (
            <div key={room.id} onClick={() => setSelectedRoomId(room.id)} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all cursor-pointer group relative overflow-hidden">
              <div className="flex items-start justify-between mb-6">
                ...
                <button className="text-gray-300 hover:text-[#0D2137] transition-colors"><MoreVertical className="w-5 h-5" /></button>
              </div>
      ...
      {/* ── Quick Update Panel ─────────────────────── */}
      {selectedRoom && <QuickUpdatePanel room={selectedRoom} onClose={() => setSelectedRoomId(null)} onSave={handleStatusUpdate} />}
    </div>
  );
}

function QuickUpdatePanel({ room, onClose, onSave }: any) {
      ...
      <div className="bg-white w-full max-w-sm rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.2)] p-8 relative z-10 animate-in slide-in-from-bottom-8 md:slide-in-from-right-8 duration-500 pointer-events-auto border border-gray-100">
        ...
        <div className="space-y-8">
          ...
          <div className="flex gap-4">
            ...
            <Button variant="outline" className="h-14 px-6 border-gray-100 rounded-xl manrope-bold text-[#0D2137] hover:bg-gray-50 transition-all">Details</Button>
          </div>
        </div>
      </div>
    </div>
